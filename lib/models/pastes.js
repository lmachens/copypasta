...
async function getPastes(query) {
  const pastesCollection = await getCollection(collectionName);
  const pastes = await pastesCollection.find(query).toArray();
  return pastes;
}

async function findAuthors(query) {
  const pastesCollection = await getCollection(collectionName);
  const result = await pastesCollection
    .aggregate([
      {
        $match: query,
      },
    ...
    .toArray();
  const authors = result.map((result) => result._id);
  return authors;
}
...
async function deletePaste(pasteId) {
  const pastesCollection = await getCollection(collectionName);
  const objectId = new ObjectID.createFromHexString(pasteId);
  const paste = await pastesCollection.deleteOne({ _id: objectId });
  if (!paste) {
    throw new Error('Can not find paste to delete');
  }
}
...
async function incrementPastaPoints(pasteId) {
  const pastesCollection = await getCollection(collectionName);
  const paste = await getPaste(pasteId);
  if (paste.pastaPoints) {
    paste.pastaPoints++;
  } else {
    paste.pastaPoints = 1;
  }
  pastesCollection.update(
    { _id: paste._id },
    { $set: { pastaPoints: paste.pastaPoints } }
  );
  return paste;
}
...
exports.incrementPastaPoints = incrementPastaPoints;
